...
function openSearchModal(){
  const o = ensureSearchModal();
  o.classList.add("active");
  setTimeout(() => qs("#food-search-text")?.focus(), 150);
}
...
function bindSearchModal(modal){
  modal.querySelector("#food-search-btn").addEventListener("click", searchByText);
  modal.querySelector("#food-barcode-btn").addEventListener("click", searchByBarcode);
  modal.querySelector("#food-search-text").addEventListener("keydown", (e) => { if(e.key === "Enter") searchByText(); });
  modal.querySelector("#food-barcode-text").addEventListener("keydown", (e) => { if(e.key === "Enter") searchByBarcode(); });
  modal.querySelector("#food-search-results").addEventListener("click", (e) => { const b = e.target.closest("[data-food-index]"); if(b) openAdd(Number(b.dataset.foodIndex)); const m = e.target.closest("#manual-product-btn"); if(m) openManual(m.dataset.barcode || ""); });
}
