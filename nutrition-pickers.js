...
function refresh(){ window.dispatchEvent(new Event("focus")); setTimeout(()=>window.dispatchEvent(new Event("focus")),400); }
...
async function submitPicker(e){
  ...
  try{
    if(type==="favorites"){
      for(const fav of selected) await addEntry(calcFavoriteEntry(fav, meal));
    }else{
      for(const tpl of selected){
        for(const item of tpl.items || []) await addEntry({date:date(), meal, name:item.name, referenceType:"templateItem", quantity:1, unit:"portion", servingName:"portion", servingWeight:null, baseCalories:n(item.calories), baseProtein:n(item.protein), baseCarbs:n(item.carbs), baseFat:n(item.fat), baseFiber:n(item.fiber), calories:n(item.calories), protein:n(item.protein), carbs:n(item.carbs), fat:n(item.fat), fiber:n(item.fiber), favorite:false, source:"mealTemplatePicker", templateName:tpl.name});
      }
    }
    o.classList.remove("active"); toast("Sélection ajoutée ✅"); refresh();
...
function init(){
  compactCards();
  setTimeout(compactCards,800);
  setInterval(compactCards,2500);
